...
def show_whatifpredict_children(n_clicks):
    if n_clicks >= 0:
        ...
        print('----------->actuals_list = ', actuals_list)
        ...
        print('FINAL WHATIF LOUT with VALUES',title,director,actor1,actor2,budget)



        x777, explainer,d,a1,a2 = wif1_preparefeatures(title, director, actor1, actor2, budget)
        ###print('INPUT FEATURE SIZE = ', x777.shape)

        ###LOAD MODEL PICKLE FOR PREDICTIONS

        loaded_model_VAG7 = pickle.load(         ###AUG27_MODEL_777_DEMO.h5   model/whatif_models/RandomForrestRegressorModel_VAVG_21082020_213610.h5
            open("data/AUG27_MODEL_777_DEMO.h5", "rb"))
        print('#######------->pkl file loaded', loaded_model_VAG7)

        ###PREDICT

        result = loaded_model_VAG7.predict(x777)
        ...
        np.random.seed(42)
        exp = explainer.explain_instance(npa, loaded_model_VAG7.predict, num_features=8)
        ...
        if (actuals_list[0] == modified_list[0] and actuals_list[1] == modified_list[1] and actuals_list[2] == modified_list[2]):
            targetval = movie_iimdbactual
        ...
        value_actual = float(movie_iimdbactual) * 10
        ...
        value_predicted = float(targetval) * 10
